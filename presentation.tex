...
\begin{tikzpicture}[scale=0.5]
	...
	\filldraw[very thick,color=orange!80!black!90, fill=orange!50!black!50!] plot[smooth cycle,tension=0.6] coordinates{(-1.5,-1) (1.0,-2) (3.5,-0.5) (2,3) (-1.2,2)};
	\filldraw[very thick,color=red!80!black!90, fill=red!50!black!50!] plot[smooth cycle,tension=0.7] coordinates{(-1.5,-1) (1.0,-1.5) (0.8,1.2) (-1.2,1)};
	\filldraw [black] (rho1) circle [radius=.05]; 
	\node [label=right:$\rho_2$] (label) at (rho2) {};
	\filldraw [black] (rho2) circle [radius=.05];
...
\begin{block}{Components}
  \begin{IEEEeqnarray*}{RL}
    \HiS =& \C^3 \ot \C^3, \ \left\{e_i\right\} \ \text{standard basis} \\
    \only<1>{P_1 &:= I \ot I - \sum_i \proj{e_i} \ot \proj{e_i} \\
      &- \proj{e_3} \ot \proj{e_1}\\
    P_2 &:= \oneo{3} \proj{\sum_i e_i \ot e_i} \\
    \rho_1 &:= \oneo{8} P_1 + \frac{3}{8} P_2  \\}
    \only<2>{\Psi &:= e_3 \ot \left( \sqrt{\frac{1+a}{2}} e_1 + \sqrt{\frac{1-a}{2}} e_3\right) \\
    \rho_2 &:= \proj{\Psi} \in \partial\S \\}
    \only<3>{\rho_a = \frac{8a}{8a + 1} \rho_1 + \oneo{9a +1} \rho_2\\
    }
  \end{IEEEeqnarray*}
\end{block}
