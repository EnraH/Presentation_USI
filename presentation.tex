...
\visible<8>{\begin{block}{Group of Entanglement Tests}
...
\begin{alertblock}{The range of separable density matrices}
  ...
  \begin{IEEEeqnarray}{RL}
    ...
    \rho^{\G} &= (\Id \ot T)\rho = \sum_{i,j} \proj{\psi_i} \ot \underbrace{\proj{\phi_j}^T}_{\proj{\phi_j^{\ast}}} \nonumber \\
...
\begin{block}{Distilling $\rho^{\ot n}$}
  ...
  \begin{IEEEeqnarray*}{RL}
    ...
    \only<3->{
      \\ &\Rightarrow \quad \rho' := P_A \ot P_B \ \rho^{\ot n} \ P_A \ot P_B \quad \only<3>{\text{is entangled}} \only<4->{\text{is NPT}}
    }
    \only<5->{
