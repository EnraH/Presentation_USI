...
\begin{block}{Security and Key}
  ...
  \end{equation}
  \visible<7->{is \alert{secure}} 
  \par
  \visible<8>{\alert{has key} if $\left\{p_{ij}\right\}= \left\{\oneo{d}\delta_{ij}\right\}$
}  
\end{block}
}
...
\begin{alertblock}{Key of private states}
  $\rho_{ABA'B'}$ is a private state \ $\Leftrightarrow$ \ $\rho_{ABE}$ has key
\end{alertblock}}
\visible<11>{
...
\visible<2>{Further: concentration of measure for randomly drawn states \ $\rightarrow$ \ conditions for dimension of respective Hilbert spaces to get PPT states / entangled states \cite{RandomEnt} \cite{RandomPPT}}
